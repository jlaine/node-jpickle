    ...
    for (var i = 0; i < pickle.length; ) {
        ...
        case REDUCE:
            ...
            if (func == 'datetime.datetime') {
                ...
                var microseconds = tmp.readUInt32BE(6) & 0xffffff;
                this.stack[this.stack.length-1] = new Date(
                    tmp.readUInt16BE(0),    // year
                    tmp.readUInt8(2) - 1,   // month
                    tmp.readUInt8(3),       // day
                    tmp.readUInt8(4),       // hour
                    tmp.readUInt8(5),       // minute
                    tmp.readUInt8(6),       // second
                    microseconds / 1000     // millisecond
                );
            } else {
                throw "Cannot emulate function: " + func;
            }
            break;
        case PUT:
